...
WifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiPhy")
    ...
    .AddTraceSource ("PhyRxDrop",
                     ...
                     "ns3::Packet::TracedCallback")
	.AddTraceSource ("PhyRxCollisionDrop",
					 "Trace source indicating a packet "
					 "has been dropped by the device because collision",
					 MakeTraceSourceAccessor (&WifiPhy::m_phyRxCollisionDropTrace),
					 "ns3::Packet::TracedCallback")
...
WifiPhy::StartReceivePreambleAndHeader (Ptr<Packet> packet, double rxPowerW, Time rxDuration)
{
  ...
  LocTag tag2;
  found = packet->PeekPacketTag(tag2);
  //found = packet->RemovePacketTag (tag2);
  if (!found)
    {
      NS_FATAL_ERROR ("No LocTag");
      return;
    }
...
WifiPhy::EndReceive (Ptr<Packet> packet, WifiPreamble preamble, MpduType mpdutype, Ptr<Event> event)
{
  ...
  LocTag tag2;
  bool found = packet->RemovePacketTag (tag2);
   if (!found)
     {
       NS_FATAL_ERROR ("No LocTag");
       return;
     }
//   if(tag2.getDistance() > 150)
//   std::cout<< tag2.getDistance() << std::endl;
  ...
  if (m_plcpSuccess == true)
    {
      ...
      if (true)//m_random->GetValue () > snrPer.per)
        {
          NotifyRxEnd (packet);
