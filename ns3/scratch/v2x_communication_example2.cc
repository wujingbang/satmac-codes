...
{   
    ...
    //printf("recv pkt, Txpos: %d,%d, Rxpos:%.1f,%.1f \n", posTx_x,posTx_y,posRx.x,posRx.y);
...
main (int argc, char *argv[])
{
    ...
    uint32_t numVeh = 900;                  // Number of vehicles
    lenCam = 100;                           // Length of CAM message in bytes [50-300 Bytes]
    double ueTxPower = 18.0;                // Transmission power in dBm
	...
	// Install constant random positions
		...
		positionAlloc->Add (Vector (900, 1000, 0));
		positionAlloc->Add (Vector (900, 1400, 0));
...
//		}
    ...
    else
    {
        ...
        ueVeh.Create (numVeh);
        ueAllNodes.Add (ueVeh);
        NS_LOG_INFO("===Loading trace file...===");
...
//    lteHelper->SetAttribute ("PathlossModel", StringValue ("ns3::ObstacleShadowingPropagationLossModel"));
//    lteHelper->SetPathlossModelAttribute("MaxDistance",UintegerValue(1000));
//
//    if (!bldgFile.empty()) {
...
//		lteHelper->SetAttribute ("FadingModel", StringValue ("ns3::ObstacleShadowingPropagationLossModel"));
//		lteHelper->SetFadingModelAttribute ("MaxDistance", UintegerValue(1000));
//	} else {
		lteHelper->SetAttribute ("PathlossModel", StringValue ("ns3::CniUrbanmicrocellPropagationLossModel"));
