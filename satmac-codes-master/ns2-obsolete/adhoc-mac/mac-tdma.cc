...
MacTdma::MacTdma(PHY_MIB_TDMA* p) :
	Mac(), mhSlot_(this), mhTxPkt_(this), mhRxPkt_(this),mhBackoff_(this), srp_ena_(0){
	...
	*/
	...
	sti = active_node_; //目前暂时以节点加入的时间顺序作为编号。
...
int MacTdma::determine_BCH(){
	int free_count=0,i=0,chosen_slot=0;
	slot_tag *fi_local_= this->collected_fi_->slot_describe;

	for(i=0 ; i < max_slot_num_; i++){
		if(fi_local_[i].busy== SLOT_FREE || fi_local_[i].busy== SLOT_COLLISION_UNKNOWN ){
			free_count++;
		}
	}
	if(free_count==0){//if there is no slot left
		return -1;
	}

	chosen_slot = Random::random()%free_count;
	for(i=0; i < max_slot_num_; i++){
		if(fi_local_[i].busy == SLOT_FREE || fi_local_[i].busy == SLOT_COLLISION_UNKNOWN){
			if(chosen_slot == 0){
				return i;
...
void MacTdma::decode_slot_tag(unsigned char* buffer,unsigned int &byte_pos,unsigned int &bit_pos, int slot_pos, Frame_info *fi){
	...
	//ptp
	value=this->decode_value(buffer,byte_pos,bit_pos,BIT_LENGTH_PTP);
	fi_local[slot_pos].ptp = (unsigned char)value;
...
void MacTdma::merge_fi(Frame_info* base, Frame_info* append, Frame_info* decision){
	...
	while(count < max_slot_num_){
		...
		if(recv_tag.busy == SLOT_FREE){
			if (count == slot_num_ && (this->node_state_ == NODE_REQUEST || this->node_state_ == NODE_WORK)){
				if(fi_local_[count].busy != SLOT_COLLISION_UNKNOWN && fi_local_[count].busy != SLOT_COLLISION_DENY){
					if (srp_ena_) {
						if (is_old_neighbor(append->sti))
							fi_local_[count].busy = SLOT_COLLISION_UNKNOWN;
					} else {
						fi_local_[count].busy = SLOT_COLLISION_UNKNOWN;
					}
				}
				//fi_local_[slot_pos].busy=SLOT_COLLISION;
			}
		}
		//Neighbor Receiving
		else if(recv_tag.busy == SLOT_BUSY && recv_tag.sti != append->sti){
			//对于BCH或者已经申请的BCH
			if ( count == slot_num_ && (node_state_ == NODE_REQUEST || node_state_ == NODE_WORK)){
				if (recv_tag.sti != sti){
					if(fi_local_[count].busy != SLOT_COLLISION_DENY){
						fi_local_[count].busy = SLOT_COLLISION_DENY;
					}
				}
			}
			//对于准备发送FI的时隙
			else if(count == slot_num_ && (node_state_ != NODE_REQUEST && node_state_ != NODE_WORK)){
				if(fi_local_[count].busy == SLOT_FREE){
					if (recv_tag.sti == sti){
						//printf("This may happen when I choose a BCH that is just collided as NEW BCH."
								//"But some neighbor just not feel the collision. And its FI still work!\n");
						//do nothing!
					}
					else{
						fi_local_[count].busy = SLOT_NEIGHBOR_RECEIVE;
					}
					//this->collision_count_++;
				}
				else if(fi_local_[count].busy == SLOT_NEIGHBOR_RECEIVE
						|| (fi_local_[count].busy == SLOT_BUSY && fi_local_[count].sti != sti)
						|| fi_local_[count].busy == SLOT_COLLISION_DENY
						|| fi_local_[count].busy == SLOT_COLLISION_UNKNOWN){
					//do nothing!
				}
				else if (fi_local_[count].busy == SLOT_BUSY && fi_local_[count].sti == sti){
					printf("I haven't send a Reservation, Why the slot is labeled as mine?");
					if(recv_tag.sti !=sti){
						fi_local_[count].busy = SLOT_COLLISION_DENY;
					}
				}
			...
			//对于普通时隙
				...
				if (recv_tag.sti == sti){
					...
					//for those has been labeled collision or NEIGHBOR_RECEIVE or busy,  no updating
					else if(fi_local_[count].busy == SLOT_NEIGHBOR_RECEIVE ||fi_local_[count].busy == SLOT_BUSY
						||fi_local_[count].busy == SLOT_COLLISION_UNKNOWN || fi_local_[count].busy == SLOT_COLLISION_DENY){
						//do nothing!
					}
				}
			}
		...
		//Neighbor Sending
			...
			if ( count == slot_num_ && (node_state_ == NODE_REQUEST || node_state_ == NODE_WORK)){
				if (recv_tag.sti != sti){
					...
					printf("I can't Receive a packet from myself in BCH");
				}
			}
			//对于准备发送FI的时隙
			else if(count == slot_num_ && (node_state_ != NODE_REQUEST && node_state_ != NODE_WORK)){
				if(fi_local_[count].busy == SLOT_FREE || fi_local_[count].busy == SLOT_NEIGHBOR_RECEIVE){
					fi_local_[count].busy = SLOT_BUSY;
				...
				if (recv_tag.sti == sti){//should not appear
					printf("Received a Reserving Message from the sti same as me, But No Reservation is Broadcasted by me!");
				}
			}
			else{
				if(fi_local_[count].busy == SLOT_FREE || SLOT_NEIGHBOR_RECEIVE){
					fi_local_[count].busy = SLOT_BUSY;
					fi_local_[count].sti = recv_tag.sti;
				}
				else if(fi_local_[count].busy == SLOT_BUSY && fi_local_[count].sti != recv_tag.sti){
					printf("this should not happen, because I can receive 2 FI in slot in simulation!");
					fi_local_[count].busy = SLOT_COLLISION_DENY;
					//this->collision_count_++;
				}
				else if(fi_local_[count].busy == SLOT_COLLISION_UNKNOWN){
					printf("this should not happen, because a Non-BCH slot can not be labeled SLOT_COLLISION_UNKNOWN!");
					fi_local_[count].busy = SLOT_COLLISION_DENY;
				}
			}
		}
...
void MacTdma::synthesize_fi_list(){
	...
	copy_frame_info_to_old();
}

void MacTdma::print_slot_status(slot_tag *fi_local) {
	int i, count;
	int free_count_ths = 0, free_count_ehs = 0;

	printf("I'm node %d, in slot %d, total %d status: ", sti, slot_count_, max_slot_num_);
	for (count=0; count < max_slot_num_; count++){
		printf("|| %d b:%d", fi_local[count].sti, fi_local[count].busy);
	}
	printf("\n");
}

void MacTdma::copy_frame_info_to_old(){
	Frame_info *fi = collected_fi_;
	slot_tag *fi_tag, *fi_tag_old;
	int count=0;
//	if( fi->slot_describe_old != NULL){
//		delete[] fi->slot_describe_old;
//	}
//	fi->slot_describe_old = new slot_tag[frame_len_];
	fi_tag_old = fi->slot_describe_old;
	fi_tag = fi->slot_describe;

	for(count = 0; count < max_slot_num_; count ++){
		fi_tag_old[count].busy = fi_tag[count].busy;
		fi_tag_old[count].psf = fi_tag[count].psf;
		fi_tag_old[count].sti = fi_tag[count].sti;
	}
//	print_slot_status(fi_tag);
//	print_slot_status(fi_tag_old);
}

bool MacTdma::is_old_neighbor(unsigned int sti) {
	slot_tag *fi_tag_old = collected_fi_->slot_describe_old;
	int count=0;
	for(count = 0; count < max_slot_num_; count ++){
		if (fi_tag_old[count].sti == sti)
			return true;
	}
	return false;
}
...
unsigned char* generate_Slot_Tag_Code(slot_tag *st){
	...
	//PTP
	MacTdma::setvalue(st->ptp, BIT_LENGTH_PTP, code, byte_pos, bit_pos);
...
Packet*  MacTdma::generate_FI_packet(){
	...
	for(int i=0; i< max_slot_num_; i++){
		if(i == this->slot_num_){
			...
			field_length = BIT_LENGTH_STI/8 ;
			if ( BIT_LENGTH_STI%8 != 0 ){
				buffer= (unsigned char)(my_sti>>( 8* field_length ));
				setvalue(buffer, BIT_LENGTH_STI%8, code, byte_pos, bit_pos);
			}
			for(int j = field_length-1 ; j >= 0 ; j-- ){
				buffer =(unsigned char)(my_sti>>(8*j));
				setvalue(buffer, 8, code, byte_pos, bit_pos);
			}
			...
			//PTP
			buffer = (unsigned char)0;
			setvalue(buffer, BIT_LENGTH_PTP, code, byte_pos, bit_pos);
		}
		else{
			//busy
			if (fi_local_[i].busy != SLOT_BUSY ){
				buffer = (unsigned char)0;
				setvalue(buffer, BIT_LENGTH_BUSY, code, byte_pos, bit_pos);
			}
			else{
			...
			//PSF
			setvalue(fi_local_[i].psf, BIT_LENGTH_PSF, code, byte_pos, bit_pos);
			//PTP
			setvalue(fi_local_[i].ptp, BIT_LENGTH_PTP, code, byte_pos, bit_pos);
		}
	}
...
void MacTdma::sendPacket(Packet *&p, int packet_type)
{
	...
	mhTxPkt_.start(p->copy(), ch->txtime_);
...
//		printf("m %.9f t[%d] _%d_ LPF %d %d %d %d %d %d %d %d %d\n",
	...
	if (slot_count_ == slot_num_){
		...
		if (node_state_ == NODE_INIT ){
			...
			else if(node_state_ == NODE_WAIT_REQUEST){

				waiting_frame_count++;

				this->clear_FI(this->collected_fi_);
				synthesize_fi_list();
				fi_collection=this->collected_fi_->slot_describe;
...
//printf("I'm node %d, in slot %d, NODE_2, %d, %d\n", this->sti, slot_count_, request_fail_times, waiting_frame_count);
				...
				if( fi_collection[slot_num_].busy == SLOT_COLLISION_UNKNOWN ||fi_collection[slot_num_].busy == SLOT_COLLISION_DENY){

					if (node_state_ == NODE_REQUEST)
						request_fail_times ++;
					else if (node_state_ == NODE_WORK && fi_collection[slot_num_].busy == SLOT_COLLISION_DENY)
						collision_count_++;
					else
						request_fail_times ++;
					waiting_frame_count ++;
					...
					if(safety_packet_queue_->Enqueue(generate_safe_packet()) >=0 ){
						...
						printf("safety_packet_queue_ is OVERRUN!!!\n");
					}

					if(node_state_ !=  NODE_WORK){
						node_state_ = NODE_WORK;
//						int offset = strlen(((CMUTrace *)this->downtarget_)->pt_->buffer());
//						if(offset!=0){
//							//((CMUTrace *)this->downtarget_)->pt_->dump();
//							offset=0;
...
int MacTdma::slot_available(int slot_num){
	...
	if(fi_found[slot_num].busy == SLOT_COLLISION_UNKNOWN ||
			fi_found[slot_num_].busy == SLOT_COLLISION_DENY || fi_found[slot_num].busy == SLOT_NEIGHBOR_RECEIVE){
		return 0;
	}
	else{
		return 1;
